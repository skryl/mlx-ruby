...
run computations written in one MLX front-end (e.g. Ruby) in another MLX
front-end (e.g. C++).
...
.. code-block:: ruby
  ...
  model.update(MLX::Utils.tree_unflatten(list(params.items())))
     return model(x)
...
on imported functions just like regular Ruby functions:
...
running them in Ruby. First, follow the :ref:`instructions <mlx_in_cpp>` to
...
Imported functions can be transformed in C++ just like in Ruby. Use
...
Ruby and importing and running them in C++:
