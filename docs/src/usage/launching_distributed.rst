...
The MLX ruby package provides two utilities to help you configure
...
.. code:: shell
    ...
    mlx.launch --hosts ip1,ip2 my_script.rb

or for testing on localhost

.. code:: shell

    mlx.launch -n 2 my_script.rb
...
connect via ssh. Moreover the input script and ruby binary need to be on each
...
* the ruby binary is available on all hosts at the same path. You can use
  ``mlx.launch --print-ruby`` to see what that path is.
...
.. code:: shell
    ...
    mlx.launch --backend mpi --mpi-arg '--mca btl_tcp_if_include en0' --hostfile hosts.json my_script.rb
