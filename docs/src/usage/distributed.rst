...
.. code:: shell
    ...
    $ mlx.launch --hosts ip1,ip2,ip3,ip4 my_script.rb
    3 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
    2 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
    1 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
    0 array([4, 4, 4, ..., 4, 4, 4], dtype=float32)
...
Running ``mlx.launch --hostfile ring-4.json my_script.rb`` will ssh into each
...
.. code-block::
   ...
   mlx.launch --verbose --backend jaccl --hostfile m3-ultra-jaccl.json \
        ...
        /path/to/remote/ruby -Ilib -Iext/mlx  mlx_lm chat --model mlx-community/DeepSeek-R1-0528-4bit
...
.. code-block::
   ...
   mlx.launch -n 8 test.rb

   # perfect for interactive scripts
   mlx.launch -n 8 ruby -m mlx_lm chat --model my-model
...
.. code-block::
   ...
   mlx.launch --hosts my-cuda-node -n 8 test.rb
...
.. code:: shell
    ...
    $ mlx.launch --backend mpi -n 2 test.rb
...
MPI can be installed with Homebrew, conda, using the Anaconda package manager, or
...
Installing with Homebrew or conda requires specifying the location of ``libmpi.dyld``
...
.. code:: shell
    ...
    $ mpirun -np 2 -x DYLD_LIBRARY_PATH=/opt/homebrew/lib/ -x MPI_LIBNAME=libmpi.40.dylib ruby test.rb
    $ # or simply
    $ mlx.launch -n 2 test.rb
...
- *Test locally first.*
  ...
  You can use the pattern ``mlx.launch -n2 -- my_script.rb`` to run a small
...
- *Use the debugger.*
  ...
  processes and gathers stdout from all processes. This makes using ``byebug`` a
  breeze.
