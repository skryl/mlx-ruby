...
* Building a custom extension and binding it to ruby.
...
in the C++ API (:ref:`cpp_ops`), and the Ruby API (:ref:`ops`) binds them.
...
* ``extensions/mlx_sample_extensions`` sets out the structure for the
  associated Ruby package
* ``extensions/bindings.cpp`` provides Ruby bindings for our operation
* ``extensions/CMakeLists.txt`` holds CMake rules to build the library and
  Ruby bindings
* ``extensions/setup.py`` holds the ```` rules to build and install
  the Ruby package

Binding to Ruby
^^^^^^^^^^^^^^^^^^

We use nanobind_ to build a Ruby API for the C++ library. Since bindings for
...
Once we have set out the CMake build rules as described above, we can add a
Ruby-native extension package using a normal gem build flow.

.. code-block:: ruby

  # Example: wire a compiled Ruby extension under ext/.
  # The extension module name and setup details are provided by that extension's
  # own extconf.rb and gemspec.
...
.. note::
    ...
    * The C++ extension library and the metal library are co-located with the ruby
      bindings and copied together if the package is installed

To build the package, first install the build dependencies with ``bundle install``.
You can then build for development using ``bundle exec rake`` (in ``extensions/``).
...
| │   └── _ext.cpython-3x-darwin.so # Ruby Binding
| ...

When you try to install using the command ``gem install .`` (in
``extensions/``), the package will be installed with the same structure as
``extensions/mlx_sample_extensions`` and the C++ and Metal library will be
copied along with the Ruby binding since they are specified as
``package_data``.
...
import the Ruby package and play with it as you would any other MLX operation.
