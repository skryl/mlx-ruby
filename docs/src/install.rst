...
MLX is available on RubyGems. All you have to do to use MLX with your own Apple
silicon computer is
...
*My OS and Ruby versions are in the required range but RubyGems still does not find
...
are using a non-native Ruby. Switch your Ruby to a native Ruby. A good
...
Similarly to the ruby library, to build and install the MLX C++ library start
...
From here follow the instructions to install either the :ref:`Ruby <ruby
...
When building either the Ruby or C++ APIs make sure to pass the cmake flag
``MLX_BUILD_CUDA=ON``. For example, to build the Ruby API run:
