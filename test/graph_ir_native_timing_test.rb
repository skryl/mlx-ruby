...
class GraphIrNativeTimingTest < Minitest::Test
  EXPORT_SCRIPT = <<~RUBY.freeze
    ...
    content = MLX::GraphIR.export_onnx_json(fun, x, y: y, model_name: "timing_probe_case")
  ...
  TRANSLATE_SCRIPT = <<~RUBY.freeze
    ...
    content = MLX::GraphIR.graph_ir_to_onnx_json(payload, model_name: "timing_probe_case")
  ...
  def test_export_onnx_json_emits_native_probe_when_enabled
    ...
    assert_match(/\[mlx\.graph_ir\.native\.timing\] export_onnx_json/, stderr)
  ...
  def test_export_onnx_json_does_not_emit_native_probe_when_disabled
    ...
    assert status.success?, "subprocess failed\nstdout:\n#{stdout}\nstderr:\n#{stderr}"
    assert_match(/\Aonnx_stub_v1\s*\z/, stdout)
    refute_match(/\[mlx\.graph_ir\.native\.timing\] export_onnx_json/, stderr)
  end

  def test_graph_ir_to_onnx_json_emits_native_probe_when_enabled
    stdout, stderr, status = Open3.capture3(
      {
        "MLX_GRAPH_IR_NATIVE_TIMING" => "1",
        "MLX_TEST_RUBY_ROOT" => RUBY_ROOT
    ...
    assert status.success?, "subprocess failed\nstdout:\n#{stdout}\nstderr:\n#{stderr}"
    assert_match(/\Aonnx_stub_v1\s*\z/, stdout)
    assert_match(/\[mlx\.graph_ir\.native\.timing\] graph_ir_to_onnx_json/, stderr)
