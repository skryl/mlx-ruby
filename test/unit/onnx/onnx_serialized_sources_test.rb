...
class Phase284GraphIrSerializedSourcesParityTest < Minitest::Test
  ...
  def test_ir_to_onnx_json_accepts_json_string_and_path_sources
    payload = exported_payload
    json = JSON.generate(payload)
    from_string = JSON.parse(MLX::ONNX.graph_ir_to_onnx_json(json, model_name: "phase284_string"))
    assert_equal "onnx_stub_v1", from_string.fetch("format")

    Dir.mktmpdir do |dir|
      path = File.join(dir, "ir.json")
      File.binwrite(path, json)
      from_path = JSON.parse(MLX::ONNX.graph_ir_to_onnx_json(path, model_name: "phase284_path"))
      assert_equal "onnx_stub_v1", from_path.fetch("format")
    end
  end

  def test_ir_to_onnx_stub_accepts_json_string_source
