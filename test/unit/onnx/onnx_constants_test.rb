...
class Phase286GraphIrConstantsParityTest < Minitest::Test
  ...
  def test_export_ir_includes_constant_values
    fun = lambda do |x|
  ...
  def test_ir_to_onnx_json_requires_constant_values
    payload = {
      "ir_version" => 1,
      "shapeless" => false,
    ...
    error = assert_raises(RuntimeError) { MLX::ONNX.graph_ir_to_onnx_json(payload) }
    assert_match(/values/, error.message)
  end

  def test_ir_to_onnx_rejects_constant_value_count_mismatch
    payload = {
      "ir_version" => 1,
      "shapeless" => false,
      "inputs" => [{ "name" => "x", "shape" => [2], "dtype" => "float32" }],
    ...
    error = assert_raises(RuntimeError) do
      Dir.mktmpdir do |dir|
        MLX::ONNX.graph_ir_to_onnx(File.join(dir, "broken.onnx"), payload)
      end
    end
