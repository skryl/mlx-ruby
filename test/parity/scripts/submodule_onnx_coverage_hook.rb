...
submodule_root = ENV["MLX_IR_COVERAGE_SUBMODULE_ROOT"].to_s
if submodule_root.empty?
  abort("MLX_IR_COVERAGE_SUBMODULE_ROOT must be set for graph-ir coverage hook")
end
...
module SubmoduleGraphIrCoverageHook
  ...
  def install!
    ...
    unless singleton.method_defined?(:__ir_coverage_original_validate!)
      singleton.alias_method :__ir_coverage_original_validate!, :validate!
    end

    singleton.define_method(:validate!) do |model_id:, inputs:, outputs:, python_bin: ENV.fetch("PYTHON_BIN", "python3")|
      capture_file = ENV["MLX_IR_COVERAGE_CAPTURE_FILE"].to_s
      if capture_file.empty?
        return __ir_coverage_original_validate!(
          model_id: model_id,
          inputs: inputs,
          outputs: outputs,
  ...
  def install_eval_hook!
    ...
    unless core_singleton.method_defined?(:__ir_coverage_original_eval)
      core_singleton.alias_method :__ir_coverage_original_eval, :eval
    end
    ...
    core_singleton.define_method(:eval) do |*args|
      ...
      __ir_coverage_original_eval(*args)
    end
  end

  def capture_enabled?
    !ENV["MLX_IR_COVERAGE_CAPTURE_FILE"].to_s.empty?
  end

  def debug?
    ENV["MLX_IR_COVERAGE_DEBUG"] == "1"
  end
  ...
  def capture_report(outputs)
    ...
    graph_payload = dot_to_ir_payload(dot_graph)
    [JSON.parse(MLX::ONNX::Native.ir_compatibility_report_json(graph_payload)), nil]
  ...
  def dot_to_ir_payload(dot_graph)
