...
OUT_FILE = PARITY_ROOT.join("graph_ir_webgpu_coverage.json").freeze
...
def model_filter
  raw = ENV["GRAPH_IR_COVERAGE_MODELS"].to_s
...
def run_submodule_coverage(spec)
  Tempfile.create(["graph_ir_webgpu_coverage", ".json"]) do |capture|
    env = unbundled_env(
      "MLX_BENCHMARK" => "1",
...
begin
  ...
  payload = {
    "format" => "graph_ir_webgpu_coverage_v1",
    "mode" => mode,
