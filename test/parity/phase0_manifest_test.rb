...
class Phase0ManifestTest < Minitest::Test
  def test_generates_manifest_with_expected_surface
    ...
    script = File.join(RUBY_ROOT, "test", "support", "parity", "generate_parity_manifest.rb")
  ...
  def test_contract_checker_accepts_generated_manifest
    ...
    generator = File.join(RUBY_ROOT, "test", "support", "parity", "generate_parity_manifest.rb")
    checker = File.join(RUBY_ROOT, "test", "support", "parity", "check_parity_manifest.rb")
