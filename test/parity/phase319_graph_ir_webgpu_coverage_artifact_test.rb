...
class Phase319GraphIrWebgpuCoverageArtifactTest < Minitest::Test
  ...
  def setup
    ...
    @tool = File.join(RUBY_ROOT, "test", "parity", "scripts", "generate_graph_ir_webgpu_coverage_report.rb")
    @out_file = File.join(RUBY_ROOT, "test", "parity", "reports", "graph_ir_webgpu_coverage.json")
  end

  def test_graph_ir_webgpu_coverage_artifact_schema_and_baseline
    first = run_tool_and_read_payload
    assert_equal "graph_ir_webgpu_coverage_v1", first.fetch("format")
  ...
  def test_graph_ir_webgpu_coverage_artifact_is_deterministic
    first = run_tool_and_read_payload
  ...
  def run_tool_and_read_payload
    env = {
      "GRAPH_IR_COVERAGE_FORCE_LOCAL" => "1",
      "GRAPH_IR_COVERAGE_MODELS" => MODELS.join(",")
    }
