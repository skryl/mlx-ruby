...
class Phase331GraphIrExportContractBoundaryParityTest < Minitest::Test
  ...
  def test_export_ir_contract_matches_between_io_and_path_targets
    fun = lambda do |x, y:|
  ...
  def test_export_ir_raw_bytes_match_between_io_and_path_targets
    fun = lambda do |x, y:|
      MLX::Core.add(MLX::Core.exp(x), y)
    end
    x = MLX::Core.array([1.0, 2.0], MLX::Core.float32)
  ...
  def test_native_export_ir_json_matches_public_export_payload
    fun = lambda do |x, y:|
      MLX::Core.add(MLX::Core.exp(x), y)
    end
    x = MLX::Core.array([1.0, 2.0], MLX::Core.float32)
    ...
    native_fun, native_args, positional_count, keyword_names =
      MLX::ONNX.send(:prepare_trace_invocation_without_native_kwargs, fun, [x], { y: y })
    native_payload = JSON.parse(MLX::ONNX::Native.export_graph_ir_json(native_fun, native_args, {}, false))
    MLX::ONNX.send(:inject_keyword_inputs!, native_payload, positional_count, keyword_names)
  ...
  def test_export_ir_tensor_info_and_keyword_entry_shapes
    fun = lambda do |x, y:|
      MLX::Core.add(x, y)
    end
    x = MLX::Core.array([1.0, 2.0], MLX::Core.float32)
  ...
  def test_export_ir_node_argument_encoding_shape
    fun = lambda do |x|
  ...
  def test_export_ir_constants_include_values_and_node_reference
    fun = lambda do |x|
  ...
  def test_ir_to_onnx_json_rejects_malformed_constants
    fun = lambda do |x|
      MLX::Core.exp(x)
    end
    x = MLX::Core.array([1.0, 2.0], MLX::Core.float32)
    payload = export_payload(fun, x)
    payload["constants"] = [{ "name" => "broken", "shape" => [1], "dtype" => "float32" }]

    error = assert_raises(RuntimeError) do
      MLX::ONNX.graph_ir_to_onnx_json(payload)
    end
    assert_match(/values|constants/i, error.message)
  end

  private

  def export_payload(fun, *args, **kwargs)
    JSON.parse(MLX::ONNX.export_graph_ir_json(fun, *args, **kwargs))
  end
