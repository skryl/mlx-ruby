...
class Phase334ExportOnnxShapelessFacadeParityTest < Minitest::Test
  ...
  def test_export_onnx_json_accepts_and_forwards_shapeless_keyword
    ...
    content = MLX::ONNX.export_onnx_json(
      fun,
      :seed,
      shapeless: true,
      opset: 19,
  ...
  def test_ir_facade_does_not_expose_removed_methods
    public_methods = MLX::ONNX.singleton_methods(false)
    REMOVED_PUBLIC_METHODS.each do |method_name|
      refute_includes public_methods, method_name
      assert_raises(NoMethodError) { MLX::ONNX.public_send(method_name) }
    end
  end

  def test_ir_public_method_surface_matches_expected_contract_exactly
    assert_equal EXPECTED_PUBLIC_METHODS.sort, MLX::ONNX.singleton_methods(false).sort
  end
end
