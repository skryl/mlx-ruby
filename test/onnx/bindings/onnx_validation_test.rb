...
class Phase281GraphIrValidationParityTest < Minitest::Test
  ...
  def test_export_ir_hash_matches_json_export_payload
    fun = lambda do |x, y:|
    ...
    from_hash = MLX::ONNX.export_graph_ir(fun, x, y: y)
    from_json = JSON.parse(MLX::ONNX.export_graph_ir_json(fun, x, y: y))

    assert_equal from_json, from_hash
  end

  def test_ir_to_onnx_json_accepts_exported_payload
    payload = exported_payload
    stub = JSON.parse(MLX::ONNX.graph_ir_to_onnx_json(payload, model_name: "phase281_case"))
    assert_equal "onnx_stub_v1", stub.fetch("format")
    assert_equal "phase281_case", stub.fetch("graph").fetch("name")
  end

  def test_ir_to_onnx_json_rejects_malformed_payload
    payload = exported_payload
  ...
  def exported_payload
    ...
    x = MLX::Core.array([1.0, 2.0], MLX::Core.float32)
    y = MLX::Core.array([3.0, 4.0], MLX::Core.float32)
    MLX::ONNX.export_graph_ir(fun, x, y: y)
  end
end
