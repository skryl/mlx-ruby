...
class Phase329GraphIrFileReadFallbackParityTest < Minitest::Test
  ...
  def test_graph_ir_to_onnx_json_accepts_path_source
    payload = graph_ir_payload
    Dir.mktmpdir do |dir|
      path = File.join(dir, "graph_ir.json")
      File.binwrite(path, JSON.generate(payload))

      stub = JSON.parse(MLX::GraphIR.graph_ir_to_onnx_json(path, model_name: "phase329_path_source"))
      assert_equal "onnx_stub_v1", stub.fetch("format")
      assert_equal "phase329_path_source", stub.fetch("graph").fetch("name")
    end
  end

  def test_graph_ir_to_onnx_json_accepts_io_source
    io = StringIO.new(JSON.generate(graph_ir_payload))
    stub = JSON.parse(MLX::GraphIR.graph_ir_to_onnx_json(io, model_name: "phase329_io_source"))
    assert_equal "onnx_stub_v1", stub.fetch("format")
