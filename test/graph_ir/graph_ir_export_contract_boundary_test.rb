...
class Phase331GraphIrExportContractBoundaryParityTest < Minitest::Test
  ...
  def test_native_export_graph_ir_json_matches_public_export_payload
    ...
    native_fun, native_args, positional_count, keyword_names =
      MLX::GraphIR.send(:prepare_trace_invocation_without_native_kwargs, fun, [x], { y: y })
    native_payload = JSON.parse(MLX::GraphIR::Native.export_graph_ir_json(native_fun, native_args, {}, false))
    MLX::GraphIR.send(:inject_keyword_inputs!, native_payload, positional_count, keyword_names)
  ...
  def test_graph_ir_to_onnx_json_rejects_malformed_constants
    fun = lambda do |x|
      MLX::Core.exp(x)
    end
    x = MLX::Core.array([1.0, 2.0], MLX::Core.float32)
