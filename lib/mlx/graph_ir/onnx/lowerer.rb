...
require_relative "lowering/elementwise"
require_relative "lowering/reduction"
require_relative "lowering/shape_ops"
require_relative "lowering/convolution"
require_relative "lowering/indexing"
  ...
  module GraphIR
    module ONNX
      module Lowerer
        ...
        def family_for(op)
          FAMILY_REGISTRY.find { |family| family.handles?(op) }
        end

        def lower_node(graph_ir:, node:, node_index:, initializers:, used_tensor_names:, known_shapes:, known_dtypes:)
          op = node.fetch("op")
...
module MLX
  module GraphIR
    ...
    def lower_onnx_node_default(node, node_index, initializers, used_tensor_names, known_shapes, known_dtypes)
      op = node.fetch("op")
    ...
    private_class_method :lower_onnx_node_default
  end
end
