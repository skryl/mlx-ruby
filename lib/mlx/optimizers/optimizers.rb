  ...
  module Optimizers
    ...
    class MultiOptimizer < Optimizer
      ...
      def split_dictionary(gradients)
        ...
        assignments = split_filter_assignments(flat_gradients)

        flat_gradients.each_with_index do |(path, grad), idx|
          parts[assignments[idx]] << [path, grad]
        end
      ...
      def split_filter_assignments(flat_gradients)
        paths = flat_gradients.map(&:first)
        if @split_filter_cache && @split_filter_cache[:paths] == paths
          return @split_filter_cache[:assignments]
        end

        fallback = @filters.length - 1
        assignments = flat_gradients.map do |path, grad|
          assigned = fallback
        ...
        @split_filter_cache = { paths: paths, assignments: assignments }
        assignments
      end
    end
    ...
    class Adafactor < Optimizer
      ...
      def apply_single(gradient, parameter, state)
        ...
        rms_update = compute_rms(update)
        one = MLX::Core.array(1.0, update.dtype)
        normalizer = MLX::Core.maximum(one, MLX::Core.divide(rms_update, clip_threshold.to_f))
        ...
        if weight_decay != 0
          decay = MLX::Core.multiply(-weight_decay.to_f, lr)
          parameter = MLX::Core.add(parameter, MLX::Core.multiply(parameter, decay))
        end
      ...
      def compute_learning_rate(current_step, parameter_rms)
        ...
        parameter_scale = if scale_parameter
          floor = MLX::Core.array(eps[1].to_f, parameter_rms.dtype)
          MLX::Core.maximum(floor, parameter_rms)
        else
          MLX::Core.array(1.0, parameter_rms.dtype)
        end
        MLX::Core.multiply(parameter_scale, relative_step_size)
      end
