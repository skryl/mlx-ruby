...
static void infer_shape_and_flatten_typed(
  ...
  if (RB_TYPE_P(value, T_ARRAY)) {
    ...
    if (shape.size() > depth + 1) {
      size_t subtree_size = 1;
      for (size_t i = depth + 1; i < shape.size(); ++i) {
        subtree_size *= static_cast<size_t>(shape[i]);
      }
      flat.reserve(flat.size() + static_cast<size_t>(len) * subtree_size);
    }
...
static mx::array call_mx_array_without_gvl(Fn&& fn) {
  ...
  auto call_without_gvl = [](void* arg) -> void* {
    auto* payload = reinterpret_cast<Payload*>(arg);
    try {
      payload->output = (*payload->fn)();
    } catch (...) {
  ...
  FnType fn_local = std::forward<Fn>(fn);
  Payload payload{&fn_local, mx::array(0.0f), nullptr};
  rb_thread_call_without_gvl(call_without_gvl, &payload, RUBY_UBF_IO, nullptr);
...
static VALUE function_wrap_vector(
  ...
  wrapper->release_gvl = release_gvl;
  wrapper->refs = refs;
  return object;
}
...
static VALUE ruby_keyword_hash_from_arrays(const mx::Kwargs& kwargs) {
  ...
  for (const auto& [key, value] : kwargs) {
    VALUE ruby_key = id_to_symbol(cached_intern_id(key.c_str()));
    rb_hash_aset(out, ruby_key, array_wrap(value));
  }
  return out;
}
...
vector_function_from_callable(VALUE callable) {
  if (!rb_respond_to(callable, symbol_ids().call)) {
...
static VALUE hash_fetch_optional(VALUE hash, const char* key) {
  ...
  VALUE symbol_key = id_to_symbol(cached_intern_id(key));
...
template <typename ValueAt>
static VALUE build_flat_ruby_array(size_t size, ValueAt value_at) {
  VALUE out = rb_ary_new_capa(static_cast<long>(size));
  for (size_t i = 0; i < size; ++i) {
    rb_ary_store(out, static_cast<long>(i), value_at(i));
  }
  return out;
}
...
static VALUE array_to_a(VALUE self) {
  ...
  if (wrapper->array.ndim() == 1) {
    wrapper->array.eval();
    const size_t size = wrapper->array.size();
    switch (wrapper->array.dtype()) {
      case mx::bool_: {
        const bool* data = wrapper->array.data<bool>();
        return build_flat_ruby_array(size, [&](size_t i) { return data[i] ? Qtrue : Qfalse; });
      }
      case mx::uint8: {
        const uint8_t* data = wrapper->array.data<uint8_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return UINT2NUM(data[i]); });
      }
      case mx::uint16: {
        const uint16_t* data = wrapper->array.data<uint16_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return UINT2NUM(data[i]); });
      }
      case mx::uint32: {
        const uint32_t* data = wrapper->array.data<uint32_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return UINT2NUM(data[i]); });
      }
      case mx::uint64: {
        const uint64_t* data = wrapper->array.data<uint64_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return ULL2NUM(data[i]); });
      }
      case mx::int8: {
        const int8_t* data = wrapper->array.data<int8_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return INT2NUM(data[i]); });
      }
      case mx::int16: {
        const int16_t* data = wrapper->array.data<int16_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return INT2NUM(data[i]); });
      }
      case mx::int32: {
        const int32_t* data = wrapper->array.data<int32_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return INT2NUM(data[i]); });
      }
      case mx::int64: {
        const int64_t* data = wrapper->array.data<int64_t>();
        return build_flat_ruby_array(size, [&](size_t i) { return LL2NUM(data[i]); });
      }
      case mx::float16: {
        const mx::float16_t* data = wrapper->array.data<mx::float16_t>();
        return build_flat_ruby_array(
            size, [&](size_t i) { return DBL2NUM(static_cast<double>(data[i])); });
      }
      case mx::bfloat16: {
        const mx::bfloat16_t* data = wrapper->array.data<mx::bfloat16_t>();
        return build_flat_ruby_array(
            size, [&](size_t i) { return DBL2NUM(static_cast<double>(data[i])); });
      }
      case mx::float32: {
        const float* data = wrapper->array.data<float>();
        return build_flat_ruby_array(
            size, [&](size_t i) { return DBL2NUM(static_cast<double>(data[i])); });
      }
      case mx::float64: {
        const double* data = wrapper->array.data<double>();
        return build_flat_ruby_array(size, [&](size_t i) { return DBL2NUM(data[i]); });
      }
      default:
  ...
  const mx::Shape shape = wrapper->array.shape();
...
static VALUE core_add(VALUE, VALUE a, VALUE b) {
  try {
    mx::array lhs = array_from_ruby(a, std::nullopt);
    mx::array rhs = array_from_ruby(b, std::nullopt);
    return array_wrap(call_mx_array_without_gvl([&]() { return mx::add(lhs, rhs); }));
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
...
static VALUE core_subtract(VALUE, VALUE a, VALUE b) {
  try {
    mx::array lhs = array_from_ruby(a, std::nullopt);
    mx::array rhs = array_from_ruby(b, std::nullopt);
    return array_wrap(call_mx_array_without_gvl([&]() { return mx::subtract(lhs, rhs); }));
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
...
static VALUE core_multiply(VALUE, VALUE a, VALUE b) {
  try {
    mx::array lhs = array_from_ruby(a, std::nullopt);
    mx::array rhs = array_from_ruby(b, std::nullopt);
    return array_wrap(call_mx_array_without_gvl([&]() { return mx::multiply(lhs, rhs); }));
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
...
static VALUE core_divide(VALUE, VALUE a, VALUE b) {
  try {
    mx::array lhs = array_from_ruby(a, std::nullopt);
    mx::array rhs = array_from_ruby(b, std::nullopt);
    return array_wrap(call_mx_array_without_gvl([&]() { return mx::divide(lhs, rhs); }));
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
...
static VALUE core_matmul(VALUE, VALUE a, VALUE b) {
  try {
    mx::array lhs = array_unwrap(a);
    mx::array rhs = array_unwrap(b);
    return array_wrap(call_mx_array_without_gvl([&]() { return mx::matmul(lhs, rhs); }));
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
...
static VALUE core_save(VALUE, VALUE file, VALUE array) {
  try {
    ...
    mx::array array_v = array_unwrap(array);

    struct CoreSavePayload {
      const std::string* file;
      const mx::array* array;
      std::exception_ptr error;
    };
    auto core_save_without_gvl = [](void* arg) -> void* {
      auto* payload = reinterpret_cast<CoreSavePayload*>(arg);
      try {
        mx::save(*payload->file, *payload->array);
      } catch (...) {
        payload->error = std::current_exception();
      }
      return nullptr;
    ...
    CoreSavePayload payload{&file_v, &array_v, nullptr};
    rb_thread_call_without_gvl(core_save_without_gvl, &payload, RUBY_UBF_IO, nullptr);
    rethrow_captured_exception(payload.error);
    return Qnil;
  } catch (const std::exception& error) {
    raise_std_exception(error);
...
static VALUE core_load(int argc, VALUE* argv, VALUE) {
  try {
    ...
    if (format_v == "npy") {
      ...
      struct CoreLoadNpyPayload {
        const std::string* file;
        mx::array output;
        std::exception_ptr error;
      };
      auto core_load_npy_without_gvl = [](void* arg) -> void* {
        auto* payload = reinterpret_cast<CoreLoadNpyPayload*>(arg);
        try {
          payload->output = mx::load(*payload->file);
        } catch (...) {
          payload->error = std::current_exception();
        }
        return nullptr;
      ...
      CoreLoadNpyPayload payload{&file_v, mx::array(0.0f), nullptr};
      rb_thread_call_without_gvl(core_load_npy_without_gvl, &payload, RUBY_UBF_IO, nullptr);
      rethrow_captured_exception(payload.error);
      return array_wrap(payload.output);
    }
    if (format_v == "npz") {
...
static VALUE core_synchronize(int argc, VALUE* argv, VALUE) {
  try {
    ...
    struct CoreSynchronizePayload {
      bool has_stream;
      mx::Stream stream;
      std::exception_ptr error;
    };
    auto core_synchronize_without_gvl = [](void* arg) -> void* {
      auto* payload = reinterpret_cast<CoreSynchronizePayload*>(arg);
      try {
        if (payload->has_stream) {
          mx::synchronize(payload->stream);
        } else {
          mx::synchronize();
        }
      } catch (...) {
    ...
    CoreSynchronizePayload payload{
        !NIL_P(stream),
        NIL_P(stream) ? mx::default_stream(mx::default_device()) : stream_unwrap(stream),
        nullptr};
    rb_thread_call_without_gvl(core_synchronize_without_gvl, &payload, RUBY_UBF_IO, nullptr);
    rethrow_captured_exception(payload.error);
    return Qnil;
  } catch (const std::exception& error) {
    raise_std_exception(error);
...
static VALUE core_compile(int argc, VALUE* argv, VALUE) {
  try {
    ...
    return function_wrap_vector(std::move(compiled), refs, true);
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
...
static VALUE ruby_shape_from_mx_shape(const mx::Shape& shape) {
  VALUE out = rb_ary_new_capa(static_cast<long>(shape.size()));
  for (size_t dim : shape) {
    rb_ary_push(out, SIZET2NUM(dim));
  }
  return out;
}

template <typename Container, typename ConvertFn>
static VALUE ruby_array_from_container(const Container& values, ConvertFn convert) {
  VALUE out = rb_ary_new_capa(static_cast<long>(values.size()));
  for (const auto& value : values) {
    rb_ary_push(out, convert(value));
  }
  return out;
}

static VALUE ruby_tensor_info_from_graph_tensor(const GraphTensorInfo& info) {
  VALUE out = rb_hash_new();
  ruby_hash_set_cstr(out, "name", ruby_string_from_std(std::get<0>(info)));
  ruby_hash_set_cstr(out, "shape", ruby_shape_from_mx_shape(std::get<1>(info)));
  ruby_hash_set_cstr(out, "dtype", ruby_string_from_std(dtype_to_string(std::get<2>(info))));
  return out;
}

static VALUE ruby_tensor_infos_from_graph_tensors(const std::vector<GraphTensorInfo>& infos) {
  VALUE out = rb_ary_new_capa(static_cast<long>(infos.size()));
  for (const auto& info : infos) {
    rb_ary_push(out, ruby_tensor_info_from_graph_tensor(info));
  }
  return out;
}

static VALUE ruby_tensor_names_from_graph_tensors(const std::vector<GraphTensorInfo>& infos) {
  VALUE out = rb_ary_new_capa(static_cast<long>(infos.size()));
  for (const auto& info : infos) {
    rb_ary_push(out, ruby_string_from_std(std::get<0>(info)));
  }
  return out;
}

static VALUE ruby_state_value_from_mx_state(const mx::StateT& value);

static VALUE ruby_state_value_from_mx_state(const mx::StateT& value) {
  if (std::holds_alternative<bool>(value)) {
    return std::get<bool>(value) ? Qtrue : Qfalse;
  }
  if (std::holds_alternative<int>(value)) {
    return INT2NUM(std::get<int>(value));
  }
  if (std::holds_alternative<size_t>(value)) {
    return SIZET2NUM(std::get<size_t>(value));
  }
  if (std::holds_alternative<float>(value)) {
    return DBL2NUM(static_cast<double>(std::get<float>(value)));
  }
  if (std::holds_alternative<double>(value)) {
    return DBL2NUM(std::get<double>(value));
  }
  if (std::holds_alternative<mx::Dtype>(value)) {
    return ruby_string_from_std(dtype_to_string(std::get<mx::Dtype>(value)));
  }
  if (std::holds_alternative<mx::Shape>(value)) {
    return ruby_array_from_container(
        std::get<mx::Shape>(value), [](size_t dim) { return SIZET2NUM(dim); });
  }
  if (std::holds_alternative<mx::Strides>(value)) {
    return ruby_array_from_container(
        std::get<mx::Strides>(value), [](auto stride) { return LL2NUM(static_cast<long long>(stride)); });
  }
  if (std::holds_alternative<std::vector<int>>(value)) {
    return ruby_array_from_container(
        std::get<std::vector<int>>(value), [](int item) { return INT2NUM(item); });
  }
  if (std::holds_alternative<std::vector<size_t>>(value)) {
    return ruby_array_from_container(
        std::get<std::vector<size_t>>(value), [](size_t item) { return SIZET2NUM(item); });
  }
  if (std::holds_alternative<std::vector<std::tuple<bool, bool, bool>>>(value)) {
    ...
    for (const auto& item : tuples) {
      ...
      rb_ary_push(row, std::get<0>(item) ? Qtrue : Qfalse);
      rb_ary_push(row, std::get<1>(item) ? Qtrue : Qfalse);
      rb_ary_push(row, std::get<2>(item) ? Qtrue : Qfalse);
      rb_ary_push(out, row);
    }
    return out;
  }
  if (std::holds_alternative<std::vector<std::variant<bool, int, float>>>(value)) {
    ...
    for (const auto& item : vars) {
      if (std::holds_alternative<bool>(item)) {
        rb_ary_push(out, std::get<bool>(item) ? Qtrue : Qfalse);
      } else if (std::holds_alternative<int>(item)) {
        rb_ary_push(out, INT2NUM(std::get<int>(item)));
      } else {
        rb_ary_push(out, DBL2NUM(static_cast<double>(std::get<float>(item))));
      }
    }
    return out;
  }
  if (std::holds_alternative<std::optional<float>>(value)) {
    ...
    if (opt.has_value()) {
      return DBL2NUM(static_cast<double>(opt.value()));
    }
    return Qnil;
  }
  return ruby_string_from_std(std::get<std::string>(value));
}

static VALUE ruby_state_values_from_mx_states(const std::vector<mx::StateT>& values) {
  VALUE out = rb_ary_new_capa(static_cast<long>(values.size()));
  for (const auto& value : values) {
    rb_ary_push(out, ruby_state_value_from_mx_state(value));
  }
  return out;
}
...
template <typename ValueAt>
static OrderedJson build_nested_json_array(
    const mx::Shape& shape,
    size_t dim,
    size_t& flat_index,
    ValueAt value_at) {
  ...
  for (size_t i = 0; i < shape[dim]; ++i) {
    out.push_back(build_nested_json_array(shape, dim + 1, flat_index, value_at));
  }
  return out;
}

template <typename ValueAt>
static OrderedJson build_flat_json_array(size_t size, ValueAt value_at) {
  OrderedJson out = OrderedJson::array();
  for (size_t i = 0; i < size; ++i) {
...
static OrderedJson json_shape_from_mx_shape(const mx::Shape& shape) {
  OrderedJson out = OrderedJson::array();
  for (size_t dim : shape) {
    out.push_back(dim);
  }
...
static OrderedJson json_scalar_from_array(const mx::array& array) {
  switch (array.dtype()) {
    case mx::bool_:
...
static OrderedJson json_values_from_array(const mx::array& source) {
  mx::array array = source;
  if (array.ndim() == 0) {
    array.eval();
    return json_scalar_from_array(array);
  }
  ...
  if (array.ndim() == 1) {
    ...
    switch (array.dtype()) {
      case mx::bool_: {
        ...
        return build_flat_json_array(size, [&](size_t i) { return OrderedJson(data[i]); });
      }
      case mx::uint8: {
  ...
  switch (flat.dtype()) {
    case mx::bool_: {
      const bool* data = flat.data<bool>();
      return build_nested_json_array(shape, 0, idx, [&](size_t i) { return OrderedJson(data[i]); });
    }
    case mx::uint8: {
      const uint8_t* data = flat.data<uint8_t>();
      return build_nested_json_array(shape, 0, idx, [&](size_t i) { return OrderedJson(data[i]); });
    ...
    case mx::float16: {
      const mx::float16_t* data = flat.data<mx::float16_t>();
      return build_nested_json_array(
          shape,
          0,
          idx,
          [&](size_t i) { return OrderedJson(static_cast<double>(data[i])); });
...
static OrderedJson json_tensor_info_from_graph_tensor(const GraphTensorInfo& info) {
  OrderedJson out = OrderedJson::object();
  out["name"] = std::get<0>(info);
  out["shape"] = json_shape_from_mx_shape(std::get<1>(info));
  out["dtype"] = dtype_to_string(std::get<2>(info));
  return out;
}

static OrderedJson json_tensor_infos_from_graph_tensors(const std::vector<GraphTensorInfo>& infos) {
  OrderedJson out = OrderedJson::array();
  for (const auto& info : infos) {
    out.push_back(json_tensor_info_from_graph_tensor(info));
  }
  return out;
}

static OrderedJson json_tensor_names_from_graph_tensors(const std::vector<GraphTensorInfo>& infos) {
  OrderedJson out = OrderedJson::array();
  for (const auto& info : infos) {
...
static OrderedJson json_state_value_from_mx_state(const mx::StateT& value);
...
static OrderedJson json_state_value_from_mx_state(const mx::StateT& value) {
  ...
  if (std::holds_alternative<mx::Shape>(value)) {
    return json_shape_from_mx_shape(std::get<mx::Shape>(value));
  }
  if (std::holds_alternative<mx::Strides>(value)) {
    OrderedJson out = OrderedJson::array();
...
static OrderedJson json_state_values_from_mx_states(const std::vector<mx::StateT>& values) {
  OrderedJson out = OrderedJson::array();
  for (const auto& value : values) {
    out.push_back(json_state_value_from_mx_state(value));
  }
  return out;
}
...
VALUE core_native_export_graph_ir(int argc, VALUE* argv, VALUE) {
  try {
    auto invocation = parse_graph_ir_export_invocation(argc, argv, "native_export_graph_ir");

    VALUE graph_inputs = rb_ary_new();
    VALUE keyword_inputs = rb_ary_new();
    VALUE graph_outputs = rb_ary_new();
    VALUE graph_constants = rb_ary_new();
    VALUE graph_nodes = rb_ary_new();
    mx::export_function(
        [&graph_inputs, &keyword_inputs, &graph_outputs, &graph_constants, &graph_nodes](
          ...
          if (record_type == "inputs") {
            ...
            if (inputs_it != data.end() &&
              ...
              graph_inputs =
                  ruby_tensor_infos_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(inputs_it->second));
            }
            return;
          }
          ...
          if (record_type == "keyword_inputs") {
            ...
            if (keywords_it != data.end() &&
                std::holds_alternative<std::vector<std::pair<std::string, std::string>>>(
                    keywords_it->second)) {
              keyword_inputs = rb_ary_new();
              const auto& keywords =
                  std::get<std::vector<std::pair<std::string, std::string>>>(keywords_it->second);
              for (const auto& [name, tensor] : keywords) {
                VALUE entry = rb_hash_new();
                ruby_hash_set_cstr(entry, "name", ruby_string_from_std(name));
                ruby_hash_set_cstr(entry, "tensor", ruby_string_from_std(tensor));
                rb_ary_push(keyword_inputs, entry);
              }
            }
            return;
          }
          ...
          if (record_type == "outputs") {
            ...
            if (outputs_it != data.end() &&
              ...
              graph_outputs =
                  ruby_tensor_infos_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(outputs_it->second));
            }
            return;
          }
          ...
          if (record_type == "constants") {
            ...
            if (constants_it != data.end() &&
              ...
              for (const auto& [name, arr] :
                ...
                VALUE entry = rb_hash_new();
                ruby_hash_set_cstr(entry, "name", ruby_string_from_std(name));
                ruby_hash_set_cstr(entry, "shape", ruby_shape_from_mx_shape(arr.shape()));
                ruby_hash_set_cstr(entry, "dtype", ruby_string_from_std(dtype_to_string(arr.dtype())));
                ruby_hash_set_cstr(entry, "values", ruby_values);
                rb_ary_push(graph_constants, entry);
              }
            }
            return;
          }
          ...
          ruby_hash_set_cstr(node, "op", ruby_string_from_std(std::get<std::string>(name_it->second)));

          VALUE node_inputs = rb_ary_new();
          auto inputs_it = data.find("inputs");
          if (inputs_it != data.end() &&
              std::holds_alternative<std::vector<GraphTensorInfo>>(inputs_it->second)) {
            node_inputs =
                ruby_tensor_names_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(inputs_it->second));
          }
          ruby_hash_set_cstr(node, "inputs", node_inputs);

          VALUE node_outputs = rb_ary_new();
          auto outputs_it = data.find("outputs");
          if (outputs_it != data.end() &&
              std::holds_alternative<std::vector<GraphTensorInfo>>(outputs_it->second)) {
            node_outputs =
                ruby_tensor_names_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(outputs_it->second));
          }
          ruby_hash_set_cstr(node, "outputs", node_outputs);

          VALUE node_arguments = rb_ary_new();
          ...
          if (arguments_it != data.end() &&
            ...
            node_arguments =
                ruby_state_values_from_mx_states(std::get<std::vector<mx::StateT>>(arguments_it->second));
          }
          ruby_hash_set_cstr(node, "arguments", node_arguments);

          rb_ary_push(graph_nodes, node);
        },
        args_kwargs_function_from_callable(invocation.fun),
        invocation.args,
        invocation.kwargs,
        invocation.shapeless);

    VALUE capture_hash = rb_hash_new();
    ruby_hash_set_cstr(capture_hash, "shapeless", invocation.shapeless ? Qtrue : Qfalse);
    ruby_hash_set_cstr(capture_hash, "inputs", graph_inputs);
    ruby_hash_set_cstr(capture_hash, "keyword_inputs", keyword_inputs);
    ruby_hash_set_cstr(capture_hash, "outputs", graph_outputs);
    ruby_hash_set_cstr(capture_hash, "constants", graph_constants);
    ruby_hash_set_cstr(capture_hash, "nodes", graph_nodes);
    return capture_hash;
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
        ...
        [&graph_inputs, &keyword_inputs, &graph_outputs, &graph_constants, &graph_nodes](
          ...
          if (record_type == "inputs") {
            ...
            if (inputs_it != data.end() &&
                std::holds_alternative<std::vector<GraphTensorInfo>>(inputs_it->second)) {
              graph_inputs =
                  json_tensor_infos_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(inputs_it->second));
            }
            return;
          }
          ...
          if (record_type == "outputs") {
            ...
            if (outputs_it != data.end() &&
                std::holds_alternative<std::vector<GraphTensorInfo>>(outputs_it->second)) {
              graph_outputs =
                  json_tensor_infos_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(outputs_it->second));
            }
            return;
          }
          ...
          if (record_type == "constants") {
            ...
            if (constants_it != data.end() &&
              ...
              for (const auto& [name, arr] : constants) {
                OrderedJson entry = OrderedJson::object();
                entry["name"] = name;
                entry["shape"] = json_shape_from_mx_shape(arr.shape());
                entry["dtype"] = dtype_to_string(arr.dtype());
                entry["values"] = json_values_from_array(arr);
          ...
          if (inputs_it != data.end() &&
              std::holds_alternative<std::vector<GraphTensorInfo>>(inputs_it->second)) {
            node_inputs =
                json_tensor_names_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(inputs_it->second));
          }
          ...
          if (outputs_it != data.end() &&
              std::holds_alternative<std::vector<GraphTensorInfo>>(outputs_it->second)) {
            node_outputs =
                json_tensor_names_from_graph_tensors(std::get<std::vector<GraphTensorInfo>>(outputs_it->second));
          }
          ...
          if (arguments_it != data.end() &&
              std::holds_alternative<std::vector<mx::StateT>>(arguments_it->second)) {
            node_arguments =
                json_state_values_from_mx_states(std::get<std::vector<mx::StateT>>(arguments_it->second));
          }
...
static VALUE core_import_function(VALUE, VALUE file) {
  try {
    ...
    return function_wrap_args_kwargs(std::move(wrapped), refs, false, true);
  } catch (const std::exception& error) {
    raise_std_exception(error);
    return Qnil;
  }
